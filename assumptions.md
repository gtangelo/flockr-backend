...
- a user can request a password reset when either logged out or logged in
- a user that is not registered cannot request a password reset
- if a user is logged in and requests a password reset, once the password is reset, the user is logged out, and prompted to log back in.
